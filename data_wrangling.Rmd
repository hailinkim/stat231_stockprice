---
title: "Data Wrangling for Shiny App"
...
#loads the stock data from Yahoo Finance from January 1 2017 and onwards
...
#get P/E ratios for each stock 
for(i in 1:length(x)) {
  #stores the data for each stock by its symbol
  dat <- getQuote(x[i], what = yahooQF("P/E Ratio")) 
  dat2 <- as.data.frame(dat) %>%
    #creates a new column with symbols
    mutate(Symbol = x[i]) %>%
    select('P/E Ratio', Symbol)
  if (i == 1) {
    pe_data <- dat2
  } else {
  pe_data <- bind_rows(pe_data, dat2)
  }
}
...
for(i in 1:length(sector)) {
  ...
  dat2 <- as.data.frame(dat) %>%
    ...
    rownames_to_column("Date") %>%
    #creates a new column with symbols
    mutate(company = sector[i])
  colnames(dat2) <- c("Date", "Open", "High", "Low", "Close", "Volume", "Adjusted", "Symbol")
  if (i == 1) {
    sector_data <- dat2
  } else {
  sector_data <- bind_rows(sector_data, dat2)
  }
}
...
#merges the data set with the table scraped from Wikipedia so it includes company
#name, sector, and P/E ratio
final <- wide_data %>%
  left_join(companies_table, by = "Symbol") %>%
  rename(Name = Security, Sector = "GICS Sector") %>%
  left_join(pe_data, by = "Symbol")

#reorders the columns so that symbol, company name and sector come first
col_order <- c(1, 953, 954, 955, 2:952)
...
write_csv(sector_data, "sector_data.csv")
write_csv(companies_table, "companies_table.csv")
```
